...
//Kang Edited
void dJointUpdateAMotorAngleInEulerMode (dJointID j)
{
	dxJointAMotor* joint = (dxJointAMotor*)j;
    ...
    amotorComputeEulerAngles(joint, axes);
}

//Kang Edited
void dJointSetAMotorAngleInEulerMode (dJointID j, dReal *a)
{
	dxJointAMotor* joint = (dxJointAMotor*)j;
    dUASSERT(joint->vtable == &__damotor_vtable,"joint is not an amotor");
...
dReal dJointGetAMotorAngleRate (dJointID j, int anum)
{
  ...
  //return 0;
    ...
    if (joint->node[0].body) {
        ...
        dReal rate = dDOT(axes[anum], joint->node[0].body->avel);
        if (joint->node[1].body) {
            rate -= dDOT(axes[anum], joint->node[1].body->avel); 
        }
        return rate;
    } else {
        return 0;
...
# endif
    ...
    if (joint->row_motor_y > 0)
        joint->motor_y.addLimot (
            joint, info, joint->row_motor_y, Midentity[0], 0);

    if (joint->row_motor_z > 0)
        joint->motor_z.addLimot (
            joint, info, joint->row_motor_z, Midentity[1], 0);
...
void dJointSetPlane2DZParam (dxJoint *joint,
                      int parameter, dReal value)
{
	dUASSERT (joint, "bad joint argument");
	dUASSERT (joint->vtable == &__dplane2d_vtable, "joint is not a plane2d");
	dxJointPlane2D* joint2d = (dxJointPlane2D*)( joint );
	joint2d->motor_z.set (parameter, value);
}
