...
void testODESpeed (dxWorld *world, dxBody * const *body, int nb,
			     dxJoint * const *_joint, int nj, dReal stepsize)
{
  int i,j,k;
...
# endif
  ...
  // - up to here - computation of world coords inv inertia 
//  return;
...
#   endif
    ...
    for (i=0; i<nj; i++) {
      ...
      Jinfo.hi = hi + ofs[i];
      Jinfo.findex = findex + ofs[i];
	  //this is where the jacobians are computed
      joint[i]->vtable->getInfo2 (joint[i],&Jinfo);
      // adjust returned findex values for global index numbering
      for (j=0; j<info[i].m; j++) {
	if (findex[ofs[i] + j] >= 0) findex[ofs[i] + j] += ofs[i];
...
#   endif
	...
	dMessage(0,"%lf %lf %lf %lf %lf %lf\n", Jsrc[0], Jsrc[1], Jsrc[2], Jsrc[4], Jsrc[5], Jsrc[6]);
...
#   endif
    ...
    for (i=0; i<nj; i++) {
      ...
      Multiply0_p81 (rhs+ofs[i],JJ,
		     tmp1 + 8*joint[i]->node[0].body->tag, info[i].m);
//	dMessage("b = -Ju - J Minv Fext - 1:\n");
//	dMessage("%lf %lf %lf\n", rhs[0], rhs[1], rhs[2]);
      if (joint[i]->node[1].body) {
	MultiplyAdd0_p81 (rhs+ofs[i],JJ + 8*info[i].m,
			  tmp1 + 8*joint[i]->node[1].body->tag, info[i].m);
      }
//	dMessage("b = -Ju - J Minv Fext - 2:\n");
//	dMessage("%lf %lf %lf\n", rhs[0], rhs[1], rhs[2]);
    }

//	dMessage("b = -Ju - J Minv Fext - almost there:\n");
//	dMessage("%lf %lf %lf\n", rhs[0], rhs[1], rhs[2]);
	...
	//done filling the matrix JMinvJ' and the rhs.
	return;


//	dMessage("b = -Ju - J Minv Fext:\n");
//	dMessage("%lf %lf %lf\n", rhs[0], rhs[1], rhs[2]);
...
//	dReal data1[8],data2[8];
//	Multiply1_8q1 (data1, JJ, lambda+ofs[i], info[i].m);
//	tprintf("force on body 2: (%lf %lf %lf)\n", data1[0], data1[1]-5*9.8, data1[2]);
...
void runTest (dxWorld *world, dxBody * const *body, int nb,
			  dxJoint * const *joint, int nj, dReal stepsize){
	testODESpeed(world,body,nb,joint,nj,stepsize);
}
